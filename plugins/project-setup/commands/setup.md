...
Step 1의 분석 결과 + 계획서(`project-plan.md`)를 조합하여 **구조화된 미리보기**를 사용자에게 보여준다.
...
- **스킬 & MCP**: 계획서 섹션 3의 검색 결과에서 "설치" 판정된 커뮤니티 스킬 + "직접 구현" 판정된 커스텀 스킬 + MCP 서버를 구분하여 나열. 섹션 3이 비어있으면 "(refine 단계 미수행 — 세팅 중 검색 실행 예정)" 표시.
...
**검색 미수행 감지 시 (하드 게이트):**
섹션 3에 "(검색 필요)", "해당 없음" 판정 없이 키워드만 있거나, 결과 열이 비어있으면:

1. **사용자에게 경고:**
   ```
   ⚠️ refine 단계에서 스킬/MCP 검색이 수행되지 않았습니다.
   커뮤니티에 검증된 스킬이 있는지 확인 없이 진행하면,
   직접 생성한 스킬이 커뮤니티 스킬보다 품질이 낮을 수 있습니다.

   지금 검색을 실행합니다.
   ```
2. `.claude/skills/skill-discovery/SKILL.md`의 검색 절차를 **반드시 실행**
3. 검색 결과를 사용자에게 보여주고 확인
4. 확인된 결과를 계획서 섹션 3에 반영
5. **검색 완료 후에만** Step 3.6으로 진행
