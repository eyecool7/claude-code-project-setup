...
#### 에이전트 vs 스크립트 역할 분담
각 워크플로우 단계에서 LLM이 직접 판단할 것과 코드로 자동화할 것을 구분.
...
**Tier 2/3 선택 시:** 에이전트 테이블을 아래 스킬 목록 뒤에 작성한다 (이름, 역할, 담당 범위).
...
- 계획서는 "무엇을, 어떤 구조로" 수준까지. CLAUDE.md/SKILL.md 파일 내용은 작성하지 않는다.
- 빈칸/TODO 남기지 않는다. 정보 없으면 [추정] 표시. (기술 결정 섹션의 "미정"은 허용)
- 범위 밖 섹션 반드시 작성 — 스코프 크리프 방지.
