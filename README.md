...
claude.ai에서 인터뷰로 기획하고, Claude Code에서 기술 심층 리뷰까지.
계획서 자체의 완성도가 다릅니다.

### 2. 계획서 하나로 4단계 완전 자동화

`/plan` → `/refine` → `/setup` → `/build` 순서대로 실행하면 프로젝트에 맞는 CLAUDE.md, rules, skills, agents, hooks가 자동 생성됩니다. 매 세션마다 계획서 기반 구축 킥오프가 제공돼요.

### 3. Tier별 확장

프로젝트 규모에 따라 작업 방식이 달라져요:

| Tier | 모드 | 적합한 경우 |
|:----:|------|-----------|
| 1 | 순차 작업 | 기능 3개 이하, 단순한 프로젝트 |
| 2 | 서브에이전트 | 독립 작업 블록이 있고 컨텍스트 절약 필요 |
| 3 | 에이전트 팀 | 대규모, 에이전트 간 소통 필요, 병렬 세션 |

### 4. 커뮤니티 스킬·MCP 자동 검색

직접 구현하기 전에 380+ 스킬 카탈로그에서 먼저 찾아요.
사용자 확인 후 설치합니다. 자동 설치는 없어요.

### 5. 세션 간 맥락 연속

기술 결정(`decisions.md`)과 삽질 기록(`lessons.md`)이 자동 축적돼요.
새 세션에서 `/build` 실행하면 이전 기록을 전부 읽고 맥락을 복구합니다.

### 6. 실수 구조적 차단

스택·의존성 충돌 자동 감지, 빠진 설정·보안 허점까지 검증 스크립트가 잡아냅니다.
Remotion + Next.js 같은 비표준 조합도 경고해줘요.
